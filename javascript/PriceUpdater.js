class PriceUpdater {
    ...
    getCurrencySign() {
      const moneyFormat = JSON.parse('"${{amount}}"');
    ...
    getDiscountVariables() {
      ...
      return {
        ...
        collectionDiscountValue: parseFloat(collectionDiscountElement.value),
        priorityDiscountValue: parseFloat(priorityDiscountElement.value),
    ...
    calculateDiscounts(variant, discountVariables) {
      const {
        discountAppliesTo, isSiteWideDiscountEnabled, isCollectionDiscountEnabled,
        collectionDiscountValue, priorityDiscountValue, sitewideDiscountValue
      } = discountVariables;
      ...
      const additonalFixedPrice = this.getAdditionalFixedPrice();
    ...
    updateDOMElements(discountData, discountVariables) {
      const {
        sellingPlan, subsavePercent, discountedPrice, discountValue, additonalFixedPrice
      } = discountData;
  
      if (sellingPlan === 'subscription') {
        this.updateSubscriptionDOM(subsavePercent, discountedPrice, discountVariables.variantPrice, discountValue, additonalFixedPrice);
      } else {
        this.updateOneTimeDOM(discountedPrice, discountVariables.variantPrice, discountValue, additonalFixedPrice);
      }
    ...
    updateSubscriptionDOM(subsavePercent, discountedPrice, variantPrice, discountValue, additonalFixedPrice) {
      ...
      updateOneTimeDOM(discountedPrice, variantPrice, discountValue, additonalFixedPrice) {
        const savingAmount = this.roundNumber(variantPrice / 100 - this.roundNumber(discountedPrice));
